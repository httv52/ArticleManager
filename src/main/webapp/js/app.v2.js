...
!function ($) {
    $(function () {
        ...
        function addMsg($msg) {
            var $el = $('.nav-user_a'), $n = $('.count:first', $el), $v = parseInt($n.text());
            $('.count', $el).fadeOut().fadeIn().text($v + 1);
            $($msg).hide().prependTo($el.find('.list-group')).slideDown().css('display', 'block');
        }

        var $msg = '<a href="#" class="media list-group-item">' + '<span class="pull-left thumb-sm text-center">' + '<i class="fa fa-envelope-o fa-2x text-success"></i>' + '</span>' + '<span class="media-body block m-b-none">' + '一个陌生通知<br>' + '<small class="text-muted">刚刚</small>' + '</span>' + '</a>';
        setTimeout(function () {
            addMsg($msg);
        }, 2500);
